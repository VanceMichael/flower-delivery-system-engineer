...
class OrderController {
  ...
  async createOrder(req, res) {
    try {
      ...
      const { items, recipient, delivery, greetingCard, payment, notes, holidayPromotion } = req.body;
      ...
      const processedItems = await Promise.all(items.map(async (item, index) => {
        ...
        try {
          if (item.productId && mongoose.Types.ObjectId.isValid(item.productId)) {
            ...
            console.log(`createOrder - found product by id:`, product ? product.name : 'not found');
          }
        } catch (e) {
        ...
        if (!product && item.productName) {
          ...
          console.log(`createOrder - found product by name:`, product ? product.name : 'not found');
        }
        
        if (!product) {
          console.log('Product not found, productId:', item.productId, 'productName:', item.productName);
      ...
      let totalAmount = processedItems.reduce((sum, item) => sum + item.finalPrice * item.quantity, 0);
      ...
      if (greetingCard) {
        const hasValidContent = (greetingCard.message && greetingCard.message.trim()) || 
      ...
      try {
        ...
        if (saveError.errors) {
          console.error('createOrder - validation errors:', JSON.stringify(saveError.errors, null, 2));
  ...
  async getOrderStatistics(req, res) {
    try {
      ...
      orders.forEach(order => {
        ...
        stats.byPaymentMethod[order.payment.method] = (stats.byPaymentMethod[order.payment.method] || 0) + 1;
      });
